...
const listEmail = (r) => {
  if (nonEmpty(r.email)) return String(r.email);
  const direct = r.PersonalEmail || r.WorkEmail || r.Email;
  if (nonEmpty(direct)) return String(direct);
  for (const [k,v] of Object.entries(r)) {
    if (String(k).toLowerCase().includes("email") && nonEmpty(v)) return String(v);
  }
  return "";
};
